...
from src.utils.config import RequestSettings
...
def test_fetch_teams_pagination(
    mock_espn_api_config, mock_teams_response_page1, mock_teams_response_page2
):
...
def test_store_teams_parquet(mock_espn_api_config, mock_teams_response_page1, temp_output_dir):
    ...
    with patch("src.ingest.teams.ParquetStorage") as mock_parquet_storage:
        # Set up mock storage
        mock_storage = mock_parquet_storage.return_value
        mock_storage.write_team_data.return_value = {
            "success": True,
