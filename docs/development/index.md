# Development Documentation

This section contains comprehensive documentation for developers working on the NCAA Basketball Prediction Model. It provides guidelines, architecture information, and detailed development resources.

## Contents

### Planning and Architecture
- [Project Structure](./project-structure.md) - Overview of directory organization
- [Pipeline Architecture](./pipeline-architecture.md) - Core data flow design
- [Modularity Guidelines](./modularity-guidelines.md) - Code organization principles
- [Implementation Plan](./implementation-plan.md) - Phase-by-phase roadmap

### Development Standards
- [Code Standards](./code-standards.md) - Coding conventions and style guide
- [Documentation Strategy](./documentation-strategy.md) - Documentation practices
- [Test Strategy](./test-strategy.md) - Testing approach and practices

### AI Development Guides
- [AI Coding Agent Guide](./ai-coding-agent-guide.md) - Guide for AI agents working on the project

### Project Management
- [GitHub Milestones](https://github.com/tim-mcdonnell/ncaa-prediction-model/milestones) - Project milestone tracking
- [GitHub Issues](https://github.com/tim-mcdonnell/ncaa-prediction-model/issues) - Task and issue tracking

## Getting Started with Development

New developers should start by:

1. Reading the [AI Coding Agent Guide](./ai-coding-agent-guide.md) for a high-level overview
2. Reviewing the [Pipeline Architecture](./pipeline-architecture.md) to understand the system design
3. Familiarizing themselves with the [Project Structure](./project-structure.md)
4. Understanding the [Test Strategy](./test-strategy.md) for development workflow

For task-specific work, refer to the individual milestone documentation and task descriptions in the respective directories. 